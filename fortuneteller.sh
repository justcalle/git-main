...
export FTTEMPDIR=$(mktemp -d /tmp/ftellerXXX)
...
grep -e message $FTTEMPDIR/fortunes_pp > $FTTEMPDIR/fortunes_messages
...
NUMMSG=$(wc $FTTEMPDIR/fortunes_messages | cut -d' ' -f2)
...
MESSAGE=$(head -n $CHOSEN $FTTEMPDIR/fortunes_messages | tail -n +$CHOSEN)

# E11 (1 point) extract only the message content using cut
MESSAGE=$(echo $MESSAGE | cut -d'"' -f4)
